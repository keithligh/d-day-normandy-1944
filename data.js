...
window.BATTLE_DATA = (function () {
  ...
  /* -- summary / sourcing notes for the in-app panel. BILINGUAL: each string carries the ---------
   *  Traditional Chinese, then the English in a <span class='en'>. The engine paints this panel with
   *  innerHTML (director.js setNotes) and the page styles `#notes .nbody .en` as the secondary
   *  language, matching how captions and map labels already render their English. The panel sits
   *  OUTSIDE the caption language toggle by design, so both languages are always shown here.
   *  The English is the same text as README.md (summary, Historical accuracy, Credits); it is reused,
   *  not re-authored, because historical content here is sourced and never invented. */
  const notes = {
    ...
    caveats:[
      ...
      "各部隊每日位置為配合敘事的示意化部署（錨定於真實地名經緯度），非逐時精確戰術圖。<span class='en'>Daily unit positions are narrative-schematic deployments anchored to real place-name coordinates, not an hour-by-hour tactical map.</span>",
      "傷亡：6月6日盟軍傷亡至少一萬人，當中四千四百一十四人確認陣亡（美軍二千五百零一、其他盟軍一千九百一十三），據美國國家D日紀念碑陣亡名錄。<span class='en'>Casualties: at least 10,000 Allied casualties on 6 June, of whom 4,414 are confirmed dead (2,501 American, 1,913 other Allied), per the U.S. National D-Day Memorial's roll of the dead.</span>",
    ],
    sources:"地理：AWS Terrarium 高程瓦片（SRTM，USGS 提供）、EOX Sentinel-2 cloudless 2016（含經修改之 Copernicus Sentinel 資料，CC BY 4.0）。史實：維基百科『Normandy landings』、Gordon A. Harrison《Cross-Channel Attack》（美國陸軍軍史中心）、Cornelius Ryan《最長的一天》、帝國戰爭博物館、美國二戰博物館、朱諾海灘中心、美國國家D日紀念碑（傷亡數字）、D-Day Center 與 D-Day Overlord 逐時時間軸（多方交叉核對）。<span class='en'>Geography: AWS Terrarium elevation tiles (SRTM, courtesy USGS); EOX Sentinel-2 cloudless 2016 (contains modified Copernicus Sentinel data, CC BY 4.0). History: Wikipedia 'Normandy landings'; Gordon A. Harrison, Cross-Channel Attack (U.S. Army Center of Military History); Cornelius Ryan, The Longest Day; the Imperial War Museums; the National WWII Museum; the Juno Beach Centre; the U.S. National D-Day Memorial (casualty figures); and the D-Day Center and D-Day Overlord hour-by-hour timelines, cross-checked against one another.</span>",
  };
